...
### 2025.2.10. 개정 자동 반영 (`analyzeFlow()` 마이그레이션 시 유지 필수)
```ts
// 해외이주비 + 영주권 신청 중 (2025.2 개정으로 미증빙 코드 강제)
if (purpose.id === 'emigration' && extras.emi_status === 'pending') {
  result.transactionCode = '거주자 지급증빙서류 미제출 지급';
  result.legalBasis = '외환규정 제4-3조 (2025.2.10. 개정으로 해외이주예정자 무한도 예외 삭제)';
}

// 거주자 재외동포 + 재산반출 (2025.2 개정으로 일반 거주자처럼 처리)
