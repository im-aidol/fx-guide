...
export default function SendGuidePage() {
  const [search, setSearch] = useState("");
  const [activeCategory, setActiveCategory] =
    useState<BusinessGuideCategory | null>(null);

  // 진입판 카테고리 카드에서 ?cat=경상거래 등으로 진입 시 자동 필터링
  useEffect(() => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <header className="mb-6">
        ...
        <h1 className="text-3xl font-bold mb-2">사유별 가이드</h1>
        <p className="text-sm text-charcoal-soft">
          고객 사유가 명확할 때 한 카드 보고 처리. 사유를 좁혀가야 하면{" "}
          <Link href="/simulator" className="text-primary font-medium hover:underline">
            당발송금 도우미
          </Link>
